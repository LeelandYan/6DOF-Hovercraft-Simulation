...
title('不同控制方法下系统阶跃响应对比', 'FontSize', 12, 'FontWeight', 'bold');
xlabel('时间 / s', 'FontSize', 11);
ylabel('系统输出与参考输入', 'FontSize', 11);
legend('BP神经网络PID控制', '参考输入', 'PSO-BPNN PID输出', '经典PID控制', ...
       'Location', 'Best');
grid on;
...
figure(3);
plot(time, error, 'r', 'LineWidth', 2);
title('系统误差随时间变化曲线', 'FontSize', 12, 'FontWeight', 'bold');
xlabel('时间 / s', 'FontSize', 11);
ylabel('误差 e(t)', 'FontSize', 11);
grid on;

% 图 4：控制量变化曲线
figure(4);
plot(time, u, 'r', 'LineWidth', 2);
title('控制量 u(t) 随时间变化曲线', 'FontSize', 12, 'FontWeight', 'bold');
xlabel('时间 / s', 'FontSize', 11);
ylabel('控制量 u(t)', 'FontSize', 11);
grid on;
...
subplot(3,1,1);
plot(time, kp, 'r', 'LineWidth', 2);
title('PID 参数随时间变化', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('比例系数 K_p', 'FontSize', 11);
grid on;

subplot(3,1,2);
plot(time, ki, 'g', 'LineWidth', 2);
ylabel('积分系数 K_i', 'FontSize', 11);
grid on;

subplot(3,1,3);
plot(time, kd, 'b', 'LineWidth', 2);
xlabel('时间 / s', 'FontSize', 11);
ylabel('微分系数 K_d', 'FontSize', 11);
grid on;
