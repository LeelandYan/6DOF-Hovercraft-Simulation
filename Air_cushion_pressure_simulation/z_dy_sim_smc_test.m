...
params_smc.mass_kg = mass_kg;
params_smc.Area_cushion_m2 = Area_cushion_m2;
params_smc.L_per_cushion_m = L_per_cushion_m;
...
% SMC控制器计算 
    % 1. 组装状态向量 [高度 z; 垂直速度 v]
    % 注意：z_curr_si 和 v_curr_si 是当前时刻的状态
    state_vec = [z_curr_si; v_curr_si];
    
    % 2. 更新动态参数 (围裙长度是变化的，取平均值传入控制器)
    % SD_curr 是 ft 单位，控制器内部计算需要 m 单位
    params_smc.current_skirt_len_m = mean(SD_curr) * ft_to_m;
    ...
    % 输入：状态、目标高度、目标速度、参数
    % 输出：目标转速 N_cmd_raw
    [N_cmd_raw, ~] = smc_heave_controller(state_vec, z_target, target_dz, params_smc);
    
    % 5. 限制命令转速范围
    N_cmd = N_cmd_raw; % 直接赋值，不再是累加 delta_N
